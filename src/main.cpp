...
#if defined(NDEBUG)
# error "Magnetocoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x64cee94f3a39e33e25d5d294d9c12e821177f7981ca9b398258aebc88133fbfb");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Magnetocoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Magnetocoin Signed Message:\n";
...
{
    // Magnetocoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 350000); // Magnetocoin: 350k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3.5 * 24 * 60 * 60; // Magnetocoin: 3.5 days
static const int64 nTargetSpacing = 5 * 60; // Magnetocoin: 5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Magnetocoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Magnetocoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xedfd9786ef0fbcc7ac47229ceb93c6094bc4e63185288f273299e3337631ecb0");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "25/04/2021 dedication never disappoints, and hardwork pays";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 90 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04499ce59b0594c06b978a533bfc83a1beccfcba3d110cf6a09a37906f1daab2b311a98c14d4e3a064ae531a1e1de17fd3b1702c691192e3706172d9c18c2b6657") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1619337760;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2085558387;

        if (fTestNet)
        {
            block.nTime    = 1619337724;
            block.nNonce   = 385552367;
        }
        ...
        assert(block.hashMerkleRoot == uint256("0x3f04ac1c2a1fba3695a7947d60056dc77c9750bb456b56e2cdaeff0b30899160"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xf5, 0xc8, 0xb3, 0xd8 }; // Magnetocoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("MagnetocoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("MagnetocoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static MagnetocoinMiner(CWallet *pwallet)
{
    printf("MagnetocoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("magnetocoin-miner");
    ...
    try { loop {
        ...
        printf("Running MagnetocoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("MagnetocoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&MagnetocoinMiner, pwallet));
}
